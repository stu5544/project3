...
// Global variable to control log level
int log_level = LOG_LEVEL_INFO;  // Set to LOG_LEVEL_INFO to hide DEBUG logs
...
void packet_handler(u_char *user_data, const struct pcap_pkthdr *pkt_header, const u_char *packet) {
    ...
    // Log debug information if log level allows
...
void broadcast_arp_response(const char *iface, const char *ip, const char *mac) {
    // Implementation for sending ARP response
}

void setup_firewall_rule(const char *ip) {
    // Implementation for setting up firewall rule
}
